function gfobj = guidedfilter_rgbd_precompute(I, r, eps)
...
gfobj.mean_I_d = boxfilter(I(:, :, 4), r) ./ gfobj.N;
...
gfobj.var_I_rd = boxfilter(I(:, :, 1).*I(:, :, 4), r) ./ gfobj.N - gfobj.mean_I_r .*  gfobj.mean_I_d; 
...
gfobj.var_I_gd = boxfilter(I(:, :, 2).*I(:, :, 4), r) ./ gfobj.N - gfobj.mean_I_g .*  gfobj.mean_I_d; 
gfobj.var_I_bb = boxfilter(I(:, :, 3).*I(:, :, 3), r) ./ gfobj.N - gfobj.mean_I_b .*  gfobj.mean_I_b;
gfobj.var_I_bd = boxfilter(I(:, :, 3).*I(:, :, 4), r) ./ gfobj.N - gfobj.mean_I_b .*  gfobj.mean_I_d;
gfobj.var_I_dd = boxfilter(I(:, :, 4).*I(:, :, 4), r) ./ gfobj.N - gfobj.mean_I_d .*  gfobj.mean_I_d;

gfobj.invSigma = cell(hei, wid);
for y=1:hei
    for x=1:wid
        Sigma = [gfobj.var_I_rr(y, x), gfobj.var_I_rg(y, x), gfobj.var_I_rb(y, x), gfobj.var_I_rd(y, x);
                 gfobj.var_I_rg(y, x), gfobj.var_I_gg(y, x), gfobj.var_I_gb(y, x), gfobj.var_I_gd(y, x);
                 gfobj.var_I_rb(y, x), gfobj.var_I_gb(y, x), gfobj.var_I_bb(y, x), gfobj.var_I_bd(y, x);
		 gfobj.var_I_rd(y, x), gfobj.var_I_gd(y, x), gfobj.var_I_bd(y, x), gfobj.var_I_dd(y, x)];
        %Sigma = Sigma + eps * eye(3);
        
        gfobj.invSigma{y, x} = inv(Sigma + eps * eye(4)); % Eqn. (14) in the paper;
    end
end
